...
class Pokemon:
    count = 0   # class variable total count of pokemon # 객체 공유
    ...
    @classmethod    # 클래스 객체로 공유한다.
    def no_pokemons(cls):
        print(f'현재 총 {cls.count}마리의 포켓몬이 생성 되었습니다.')
...
class Pikachu(Pokemon):  # class 자식클래스(부모클래스):
    ...
    def __repr__(self):
        return f'이 피카츄는 {len(self.skills)}개의 스킬을 가지고 있는 {self.owner}의 피카츄입니다.'  # toString 이랑 비슷
...
if __name__ == "__main__":
    pa = Pikachu('한지우', '번개/50만볼트')
    pb = Pikachu('아이리스', '번개/50만볼트/100만볼트')
